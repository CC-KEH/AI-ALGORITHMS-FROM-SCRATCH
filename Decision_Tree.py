...
class Decision_Tree:
    ...
    def calculate_entropy(self,y):
        # Keeping the frequency of each value from 0 to the largest value in y 
        hist = np.bincount(y)
        ps = hist / len(y)
        
        return -np.sum([p * np.log(p) for p in ps if p>0]) # Here p>0 is for those values that are not present in y
